...
class TestQLearning:
    def test_td_update_computation(self):
        """Q-learning TD-error: Q[s,a] += alpha * (r + gamma * max_a'(Q[s',a']) - Q[s,a])"""
        policy = EpsilonGreedyPolicy(DummyEnv2(), epsilon=0.0, seed=0)
        agent = TDAgent(
            DummyEnv2(), policy, alpha=0.5, gamma=0.9, algorithm="qlearning"
        )

        agent.Q[0][1] = 2.0
        agent.Q[1][0] = 0.3
        agent.Q[1][1] = 1.8  # max Q[s'] = 1.8

        new_q = agent.update_agent(make_batch(0, 1, 1.0, 1, False))

        expected = 2.0 + 0.5 * ((1.0 + 0.9 * 1.8) - 2.0)
    ...
    def test_predict_action_greedy_vs_random(self):
        ...
        agent0 = TDAgent(
            DummyEnv5(), policy0, alpha=0.1, gamma=0.9, algorithm="qlearning"
        )
        ...
        agent1 = TDAgent(
            DummyEnv5(), policy1, alpha=0.1, gamma=0.9, algorithm="qlearning"
        )
    ...
    def test_uses_max_not_on_policy_action(self):
        """Q-learning bootstraps from max Q[s'], not the epsilon-greedy action."""
        policy = EpsilonGreedyPolicy(DummyEnv2(), epsilon=0.0, seed=0)
        agent = TDAgent(
            DummyEnv2(), policy, alpha=0.5, gamma=0.9, algorithm="qlearning"
        )
        ...
        agent.Q[0][1] = 2.0
        agent.Q[1][0] = 0.3  # greedy action (argmax) but low value
        agent.Q[1][1] = 1.8  # higher value — max should be taken here

        new_q = agent.update_agent(make_batch(0, 1, 1.0, 1, False))

        expected_with_max = 2.0 + 0.5 * ((1.0 + 0.9 * 1.8) - 2.0)
        wrong_with_greedy = 2.0 + 0.5 * ((1.0 + 0.9 * 0.3) - 2.0)
        assert new_q == pytest.approx(expected_with_max)
        assert new_q != pytest.approx(wrong_with_greedy)
